...
def infer_depth(image_processor, model, frame_bgr, device):
    ...
    post = image_processor.post_process_depth_estimation(
        outputs,
        source_sizes=[(pil.height, pil.width)],
...
def run():
    ...
    while True:
        ...
        t0 = time.time()
        depth = infer_depth(proc, model, frame, device)
        depth_gray = normalize_for_viz(depth, NORMALIZE_MODE, ABS_MIN_M, ABS_MAX_M)
        ...
        put_text(
            fused, f"FPS: {ema_fps:.1f}", (15, fused.shape[0] - 20), 0.8, (20, 220, 20)
        )

        cv2.imshow("ZoeDepth | Izquierda: RGB  |  Derecha: Profundidad", fused)
